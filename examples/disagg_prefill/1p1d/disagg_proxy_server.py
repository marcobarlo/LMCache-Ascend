...
async def lifespan(app: FastAPI):
    ...
    if pd_buffer_admission_enabled:
        ...
        if pd_capacity_slots < 4:
            logger.warning(
                "PD buffer admission capacity (%d slots) is too small for"
                " model %s: any prompt longer than %d tokens will fail with"
                " 'Requested N slots exceeds total capacity'. Pass"
                " --pd-transfer-mode delay_pull to disable proxy admission"
                " control, or raise --pd-buffer-size / --chunk-size to match"
                " the decoder's LMCache PD buffer config.",
                pd_capacity_slots,
                global_args.model,
                pd_capacity_slots * global_args.chunk_size,
            )
    else:
        logger.info(
    ...
    try:
        ...
        async def generate_stream():
            ...
            try:
                ...
                # TCP chunks may split multi-byte UTF-8 characters across
                # chunk boundaries; an incremental decoder buffers the
                # partial tail bytes until the next chunk arrives.
                utf8_decoder = codecs.getincrementaldecoder("utf-8")()
                async for chunk in stream_service_response(
                    decode_client.client, "/v1/completions", decode_req_data
                ):
                    chunk_str = utf8_decoder.decode(chunk)
