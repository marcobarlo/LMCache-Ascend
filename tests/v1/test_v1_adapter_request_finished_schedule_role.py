...
This file is adapted from the upstream #3340 test for
``LMCacheAscendConnectorV1Impl``, whose ``request_finished`` fully
overrides the upstream 0.4.5 body (pinned by CI) with the #3340 fix.
The fixtures therefore pin the extra attributes the Ascend override
reads (``store_async`` / ``kv_role``) and the extra engine methods it
calls on the abort path (``lookup_unpin`` /
...
_ADAPTER_LOGGER_NAME = "lmcache_ascend.integration.vllm.vllm_v1_adapter"
...
class _FakeEngine:
    def __init__(self) -> None:
        ...
        self.waited: list[set[str]] = []

    def lookup_unpin(self, req_id: str) -> None:
        self.unpinned.append(req_id)

    def wait_for_pending_stores(self, req_ids: set[str]) -> set[str]:
        self.waited.append(req_ids)
        return req_ids
...
def _make_connector(
    *,
    ...
    the connector that delegate to ``self._manager``, so the fixture
    ...
    connector.config = SimpleNamespace(
        ...
        enable_chunk_hashes_return=False,
    )
...
def test_request_finished_non_aborted_returns_chunk_hashes() -> None:
    """Ascend-only extension (PR #237): when
    ``enable_chunk_hashes_return`` is set, ``return_params`` must carry
    ``chunk_hashes`` fetched from the innermost lookup client, and no
    abort cleanup must fire on a non-aborted finish.
    """
    ...
    assert delay_free is False
    assert return_params == {"chunk_hashes": ["h1", "h2"]}
    assert engine.storage_manager.cancelled == []
