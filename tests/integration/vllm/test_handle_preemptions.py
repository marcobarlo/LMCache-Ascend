...
def test_ascend_adapter_accepts_connector_metadata_arg():
    """Newer vLLM passes the metadata object; ids come from preempted_req_ids."""
    pytest.importorskip("lmcache")
    pytest.importorskip("vllm")
    adapter_mod = pytest.importorskip("lmcache_ascend.integration.vllm.vllm_v1_adapter")
    ...
    # Emulate LMCacheConnectorMetadata built by build_connector_meta()
    ...
    lmcache_engine.lookup_unpin.assert_any_call("req-1")
    lmcache_engine.lookup_unpin.assert_any_call("req-2")
    lmcache_engine.wait_for_pending_stores.assert_called_once_with(
        {"req-1", "req-2"}
    )


def test_ascend_adapter_ignores_metadata_without_preempted_ids():
    """Metadata lacking preempted_req_ids must not crash (upstream no-op)."""
    pytest.importorskip("lmcache")
    pytest.importorskip("vllm")
    adapter_mod = pytest.importorskip("lmcache_ascend.integration.vllm.vllm_v1_adapter")
